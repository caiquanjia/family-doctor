...
function openIDB() {
  return new Promise((resolve, reject) => {
    if (_idb) {
      console.log('[IndexedDB] reused existing DB');
      return resolve(_idb);
    }
    // ★ V1.8.2: 版本号从 1 升到 2，确保已存在的数据库也会触发 onupgradeneeded 创建 originalFiles
    console.log('[IndexedDB] opening healthAssistDB v2...');
    const req = indexedDB.open('healthAssistDB', 2);
    req.onupgradeneeded = e => {
      console.log('[IndexedDB] onupgradeneeded fired, oldVersion:', e.oldVersion);
      const db = e.target.result;
      // 从旧版本升级时，如果对象仓库不存在则创建
      if (!db.objectStoreNames.contains('originalFiles')) {
        db.createObjectStore('originalFiles', { keyPath: 'kbId' });
        console.log('[IndexedDB] originalFiles store created');
      } else {
        console.log('[IndexedDB] originalFiles store already exists');
      }
    };
    req.onsuccess = e => {
      _idb = e.target.result;
      console.log('[IndexedDB] opened successfully, store exists:', _idb.objectStoreNames.contains('originalFiles'));
      resolve(_idb);
    };
    req.onerror = e => {
    ...
    req.onblocked = e => {
      console.error('[IndexedDB] open blocked:', e);
    };
  });
}

async function saveOriginalFile(kbId, fileName, fileType, fileData) {
  console.log('[IndexedDB] saveOriginalFile called, kbId:', kbId, 'fileName:', fileName, 'size:', fileData ? fileData.byteLength : 'null');
  const db = await openIDB();
  console.log('[IndexedDB] db opened, store exists:', db.objectStoreNames.contains('originalFiles'));
  return new Promise((resolve, reject) => {
    try {
      const tx = db.transaction('originalFiles', 'readwrite');
      const store = tx.objectStore('originalFiles');
      const request = store.put({ kbId, fileName, fileType, fileData, uploadDate: new Date().toISOString() });
      request.onsuccess = () => {
        console.log('[IndexedDB] save success for kbId:', kbId);
        resolve();
      };
      request.onerror = e => {
        console.error('[IndexedDB] save request error:', e.target.error);
        reject(e.target.error);
      };
      tx.onerror = e => {
      ...
      console.error('[IndexedDB] save exception:', err);
      reject(err);
    }
  });
}

async function getOriginalFile(kbId) {
  console.log('[IndexedDB] getOriginalFile called, kbId:', kbId);
  const db = await openIDB();
  return new Promise((resolve, reject) => {
    try {
      ...
      req.onsuccess = () => {
        console.log('[IndexedDB] get result for kbId:', kbId, 'found:', !!req.result, 'size:', req.result && req.result.fileData ? req.result.fileData.byteLength : 0);
        resolve(req.result);
      };
      req.onerror = e => {
        console.error('[IndexedDB] get request error:', e.target.error);
        reject(e.target.error);
      };
    } catch (err) {
...
async function deleteOriginalFile(kbId) {
  const db = await openIDB();
  return new Promise((resolve, reject) => {
    const tx = db.transaction('originalFiles', 'readwrite');
    const store = tx.objectStore('originalFiles');
...
async function handleFilesSelected(input) {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    try {
      let rawText = '';
      let totalPages = 0;
      let rawFileBuffer = null;  // ★ V1.8: 保留原始文件数据

      if (ext === 'txt' || ext === 'csv') {
        rawText = await readFileAsText(file);
      } else if (ext === 'pdf') {
        rawFileBuffer = await file.arrayBuffer();
        // ★ V1.8.4: pdf.js 会消费/转移 ArrayBuffer，导致后续无法存入 IndexedDB
        // 传入一个副本，保留原始 buffer 给 IndexedDB 使用
        const pdfBuffer = rawFileBuffer.slice(0);
        const pdfResult = await extractPdfText(pdfBuffer);
        rawText = pdfResult.text;
        totalPages = pdfResult.totalPages;
      } else if (ext === 'docx') {
        rawFileBuffer = await file.arrayBuffer();
        // ★ V1.8.4: 同上，保留原始 buffer，解析用副本
        const docxBuffer = rawFileBuffer.slice(0);
      ...
      // ★ V1.8: 保存原始文件数据（用于原文查看）
      _uploadState.fileResults.push({ fileName: file.name, entries: entries, error: null, totalPages, rawFileBuffer, fileType: ext });
    } catch (err) {
...
async function extractPdfText(arrayBuffer) {
  ...
  for (let i = 1; i <= total; i++) {
    ...
    // ★ V1.8: 每页之间插入页码标记，供 autoSplitEntries 解析
    fullText += `【PAGE:${i}】${pageText}\n\n`;
  }
  return { text: fullText, totalPages: total };
}
...
// 步骤5：确认保存（每个文件创建独立KB） — V1.8: async（需要写IndexedDB）
async function confirmUploadSave() {
  // 从编辑框读取最终值
  const successResults = _uploadState.fileResults.filter(r => !r.error && r.entries.length > 0);
  successResults.forEach((result, fi) => {
    result.entries.forEach((e, ei) => {
  ...
  for (const result of successResults) {
    ...
    ov.customKbs.push({
      ...
      totalPages: result.totalPages || 0  // ★ V1.8: 保存总页数
    });
    savedCount++;
    kbExpandedState[newId] = true;

    // ★ V1.8: 保存原始文件到 IndexedDB（用于原文查看）
    if (result.rawFileBuffer) {
      console.log('[Upload] rawFileBuffer exists, size:', result.rawFileBuffer.byteLength, 'saving for kbId:', newId);
      try {
        await saveOriginalFile(newId, result.fileName, result.fileType || 'pdf', result.rawFileBuffer);
        console.log('[Upload] original file saved successfully');
      } catch(e) {
        console.error('[Upload] saveOriginalFile failed:', e);
        showToast('⚠️ 原始文件保存失败，原文查看可能不可用：' + e.message);
      }
    } else {
      console.warn('[Upload] rawFileBuffer missing!');
      showToast('⚠️ 未获取到原始文件数据，无法支持原文查看');
    }
  }
...
function autoSplitEntries(rawText, fileName) {
  ...
  for (const line of rawLines) {
    ...
    if (pageMatch) {
      ...
      const rest = line.replace(/【PAGE:\d+】/, '').trim();
      if (rest) {
        cleanLines.push(rest);
        pageMarkers.push(currentPage);
      }
    } else if (line.trim()) {
      cleanLines.push(line.trim());
      pageMarkers.push(currentPage || 1); // 非PDF文本默认页码1
    }
  }
  ...
  let currentContent = '';
  let currentKeywords = [];
  let currentPageStart = 0;  // ★ V1.8: 当前条目起始页码
  let currentPageEnd = 0;    // ★ V1.8: 当前条目结束页码
  ...
  function flushEntry() {
    if (currentTitle && currentContent.trim()) {
      entries.push({
        ...
        source: '',
        builtIn: false,
        pageRange: currentPageStart > 0 ? { start: currentPageStart, end: currentPageEnd } : null  // ★ V1.8
      });
    }
    currentTitle = '';
    currentContent = '';
  ...
  // ★ V1.8: 行索引映射（cleanLines 和 pageMarkers 同长度）
  let lineIdx = 0;

  // 拆分逻辑
  for (let li = 0; li < lines.length; li++) {
    const trimmed = lines[li];
    const pg = pageMarkers[li] || 0;
    ...
    if (isHeading(trimmed)) {
      ...
      if (pg > 0) {
        if (currentPageStart === 0) currentPageStart = pg;
        currentPageEnd = pg;
      }
    }
  }
...
async function openSourceViewer(kbId, entryId) {
  // 查找条目信息
  const ov = getKbOverlay();
  let entry = null;
  let kbName = '';
  ...
  for (const kb of (ov.customKbs || [])) {
    if (kb.id === kbId) {
      kbName = kb.name;
      kbType = kb.kbType || 'rag_common';
      entry = (kb.entries || []).find(e => e.id === entryId);
      if (entry) { pageRange = entry.pageRange; break; }
    }
  }

  // 再在内置KB的added中查找
  if (!entry) {
    for (const kbId2 of ['ws', 'drugs']) {
      const ed = ov.edits[kbId2];
      entry = (ed.added || []).find(e => e.id === entryId);
      if (entry) { kbName = kbId2 === 'ws' ? 'WS/T 810-2022 急诊转诊标准' : '药品目录'; kbType = kbId2 === 'ws' ? 'rag_literature' : 'rag_common'; break; }
    }
  }

  // ★ V1.8.1: 搜索内置KB的原始条目（flattenWsKB / flattenDrugsKB）— 修复遗漏
  if (!entry) {
    if (kbId === 'ws') {
      const wsEntries = flattenWsKB();
      entry = wsEntries.find(e => e.id === entryId);
      if (entry) {
      ...
      const drugsEntries = flattenDrugsKB();
      entry = drugsEntries.find(e => e.id === entryId);
      if (entry) {
        kbName = '药品目录';
        kbType = 'rag_common';
        pageRange = entry.pageRange;
  ...
  // ★ V1.8.1: 如果仍找不到，尝试从 _lastRagEntries 中获取元信息，展示至少有标题和来源
  if (!entry) {
    const ragEntries = window._lastRagEntries || [];
    const ragEntry = ragEntries.find(e => e.kbId === kbId && e.entryId === entryId);
    if (ragEntry) {
      // 从 _lastRagEntries 构造一个最小条目对象，至少能展示标题和来源
      entry = {
        id: entryId,
        title: ragEntry.title || '（条目详情未缓存）',
        content: '该条目内容在当前会话中未完整缓存。请重新提问以获取完整引用信息。',
        source: ragEntry.source || '',
        actions: []
      };
      kbName = ragEntry.kbName || kbId;
      kbType = ragEntry.kbType || 'rag_common';
      pageRange = ragEntry.pageRange;
    }
  }

  if (!entry) { showToast('未找到该条目'); return; }

  // 尝试从 IndexedDB 加载原始文件
  const origFile = await getOriginalFile(kbId);

  // 创建模态框
  const overlay = document.createElement('div');
  overlay.id = 'sourceViewerOverlay';
  overlay.className = 'kb-modal-overlay';

  const pageLabel = pageRange ? (pageRange.start === pageRange.end ? `第${pageRange.start}页` : `第${pageRange.start}-${pageRange.end}页`) : '';
  const sourceLabel = entry.source || kbName;

  // 构建模态框HTML
  let modalHtml = `<div class="source-viewer-modal" onclick="event.stopPropagation()">
    <div class="source-viewer-header">
      <div class="source-viewer-title">📖 引用详情</div>
      <button class="kb-panel-close" onclick="closeSourceViewer()">✕</button>
    </div>
    <div class="source-viewer-body">
      <div class="source-viewer-meta">
        <span class="source-tag">${kbType === 'rag_literature' ? '📄 文献类' : kbType === 'rag_common' ? '💡 常识类' : '📊 结构化'}</span>
        <span style="font-weight:600;margin-left:8px;">${kbName}</span>
        ${pageLabel ? `<span style="color:#5b86e5;margin-left:8px;">${pageLabel}</span>` : ''}
      </div>

      ${origFile ? `
      <div class="source-viewer-original" id="sourceOriginalView" style="display:block;">
        <div class="source-original-header">📄 原始文件预览</div>
        <div class="source-entry-title" style="margin-top:8px;">📌 ${entry.title}</div>
        <div id="sourcePdfContainer" style="max-height:400px;overflow-y:auto;"></div>
      </div>
      ` : `
      <div class="source-viewer-content">
        <div class="source-entry-section">
          <div class="source-entry-title">📌 ${entry.title}</div>
          <div class="source-entry-content">${entry.content.replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\n/g, '<br>')}</div>
        </div>
      </div>
      `}
    </div>

    <div class="source-viewer-footer">
      ${origFile ? `<button class="kb-mini-btn kb-mini-btn-primary" onclick="toggleOriginalView()">📄 隐藏/显示原文</button>` : '<span style="color:#999;font-size:12px;">原始文件未保存（仅显示条目文本，建议删除该知识库后重新上传）</span>'}
      <div style="font-size:12px;color:#666;">📚 来源：${sourceLabel} ${pageLabel ? '（' + pageLabel + '）' : ''}</div>
    </div>
  </div>`;

  overlay.innerHTML = modalHtml;
  overlay.onclick = closeSourceViewer;
  document.body.appendChild(overlay);

  // 如果有PDF原件，预渲染
  if (origFile && origFile.fileType === 'pdf') {
    try {
      const pdf = await pdfjsLib.getDocument({ data: origFile.fileData }).promise;
      const startPage = pageRange ? pageRange.start : 1;
      const endPage = pageRange ? Math.min(pageRange.end, pdf.numPages) : Math.min(startPage + 2, pdf.numPages);

      const container = document.getElementById('sourcePdfContainer');
      for (let p = startPage; p <= endPage; p++) {
        ...
        canvas.style.border = p === startPage ? '2px solid #5b86e5' : '1px solid #ddd';
        ...
        label.style.cssText = 'font-size:11px;color:#666;text-align:center;margin-bottom:4px;';
...
function closeSourceViewer() {
  const overlay = document.getElementById('sourceViewerOverlay');
  if (overlay) overlay.remove();
}

function toggleOriginalView() {
  const view = document.getElementById('sourceOriginalView');
